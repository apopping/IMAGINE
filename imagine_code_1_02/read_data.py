...
def read_data(args, obs_par):
	...
	print(file_path)
        if not os.path.isfile(file_path):
            print("WARNING: The file {file_path} does not exist!")
            print("Continueuing script with next file if possible ...")
        else:
            if not os.path.isdir('temp_links'): # make a softlink to the workdirectory
            	os.system('mkdir temp_links')
            command = 'ln -s ' + file_path + ' temp_links/' + file.strip('\n')
            print(command)
            os.system(command)
    print('all files in temp_links')
    print(os.system('ls temp_links/'))
    ...
    if args.mode == 'cont':
        command = 'atlod in=temp_links/* out=temp_data/data  ifsel=' + str(obs_par['cont_if']) + ' options=bary,birdie,rfiflag,noauto edge=20'
