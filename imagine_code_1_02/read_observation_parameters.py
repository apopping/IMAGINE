...
def read_observation_parameters(args, db_file):
    ...
    '''  #comment this part out as it has nothing to do with continuum data and is coming up with a ngc1512
    # read other relevant parameters for the galaxy
    cur.execute("SELECT vel,vel_min,vel_max FROM galaxy WHERE target=" + obs_par['target']) #there is suppose to be a repr(obs_par['target'])
